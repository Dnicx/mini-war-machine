...
const PHASE_PATTERNS: Record<Phase, RegExp[]> = {
  'Start of Game': [/start of (the )?game/i, 
    /before the battle/i, 
    /pre-battle/i, 
    /deployment/i, 
    /model can be attached/i, 
    /capacity of/i
  ],
  
  'Start of Battle Round': [/start of (the )?(battle )?round/i, 
    /start of (your )?turn/i, 
    /beginning of (the )?(battle )?round/i
  ],
...
const TIMING_PATTERNS: Record<Timing, RegExp[]> = {
  ...
  beforeTarget: [/before/i, 
    /when selecting/i, 
    /when choosing/i
  ],
  
  afterTargeted: [/after/i, 
    /when targeted/i, 
    /when hit/i
  ],
...
const ONCE_PER_BATTLE = [/once per battle/i, 
  /one use only/i]
const ONCE_PER_ROUND = [/once per battle round/i, 
  /once per turn/i]
